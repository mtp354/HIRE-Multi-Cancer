...
def select_cohort(config: Config, birthyear=None):
    ...
    CANCER_INC.query('Sex == @config.COHORT_SEX & Race == @config.COHORT_RACE & Cohort == @birthyear', inplace=True)
    ...
    if CANCER_INC['Age'].max() < 83:
        ...
        while CANCER_INC['Age'].max()<83:
            ...
            cancer_inc = pd.read_csv(f'{PATHS["incidence"]}Incidence.csv')
            cancer_inc = cancer_inc[cancer_inc['Site'].isin(config.CANCER_SITES)]  # keeping the cancers of interest
            cancer_inc.query('Sex == @config.COHORT_SEX & Race == @config.COHORT_RACE & Cohort == @cohort_year', inplace=True)
    ...
    MORT.query('Sex == @config.COHORT_SEX & Race == @config.COHORT_RACE & Cohort == @birthyear', inplace=True)
    SURV.query('Sex == @config.COHORT_SEX & Race == @config.COHORT_RACE', inplace=True)
    SURV_ed.query('Sex == @config.COHORT_SEX & Race == @config.COHORT_RACE', inplace=True)
    ...
    CANCER_PDF = 0.002 * np.ones(config.END_AGE - config.START_AGE + 1)  # starting from 0 incidence and using bias optimization
    CANCER_PDF[:min(config.END_AGE - config.START_AGE + 1, max(0, 35 - config.START_AGE))] = 0.0 # set ages before 35 to 0.0
    CANCER_PDF_lst = []
    ...
    if config.LOAD_LATEST:
        if len(config.CANCER_SITES) == 1: # 1 cancer site
            ...
            list_of_files = glob.glob(f'{PATHS["calibration"]}*{config.COHORT_SEX}_{config.COHORT_RACE}_*{config.CANCER_SITES[0]}_*.npy')
            if len(list_of_files) == 0: # Check if there is a previous numpy file matching the same sex and cancer site
                list_of_files = glob.glob(f'{PATHS["calibration"]}*{config.COHORT_SEX}_*{config.CANSCER_SITES[0]}_*.npy')
            if len(list_of_files) == 0: # Check if there is a previous numpy file matching the same race and cancer site
                list_of_files = glob.glob(f'{PATHS["calibration"]}*{config.COHORT_RACE}_*{config.CANCER_SITES[0]}_*.npy')
            if len(list_of_files) == 0: # Check if there is a previous numpy file matching the same cancer site
                list_of_files = glob.glob(f'{PATHS["calibration"]}*{config.CANCER_SITES[0]}_*.npy')
            ...
            CANCER_PDF = np.load(latest_file)[config.START_AGE:config.END_AGE+1]
        else: # multiple cancer sites
            CANCER_PDF_lst = []
            for i in range(len(config.CANCER_SITES)):
                # Check if there is a previous numpy file matching the same sex and race and cancer site
